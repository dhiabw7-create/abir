...
import { FileText, Home, LogOut, Settings, UserRound } from "lucide-react";
...
const SidebarPation = ({ isOpen, onClose }) => {
  const { user, logout } = useAuth();
  const navigate = useNavigate();
  const displayName = user?.name || [user?.prenom, user?.nom].filter(Boolean).join(" ") || "Pation";
      ...
      >
        <div className="flex h-full flex-col p-4">
          ...
          <nav className="mt-6 flex-1 space-y-2">
            <SidebarItem to="/pation/dashboard" icon={<Home size={18} />} label="Mon espace" />
            <SidebarItem to="/pation/ordonnances" icon={<FileText size={18} />} label="Mes ordonnances" />
            <SidebarItem to="/profile" icon={<UserRound size={18} />} label="Profil" />
